...
with tf.name_scope("Init"):
    ...
    train_writer = tf.summary.FileWriter('./Graph_run_4/train',sess.graph)
    test_writer = tf.summary.FileWriter('./Graph_run_4/test',sess.graph)
...
with tf.Session() as sess:
    ...
    for [images, texts], labels_input in training_generator:
        ...
        if count%20 == 0 or count < 10:
            ...
            summary, _ = sess.run([merged,minimizer], feed_dict=feed_dicty)
            train_writer.add_summary(summary, count)
        if count%5000 == 0:
            save_path = saver.save(sess, "models/run_4/model"+str(count)+".ckpt")
