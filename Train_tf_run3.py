...
#optimizer = tf.contrib.opt.AdamWOptimizer(learning_rate=learning_rate, weight_decay=0.01,name='Adam-op')
...
#    saver.restore(sess, "models/run_1/model30000.ckpt")
    ...
    for [images, texts], labels_input in training_generator:
        ...
        #    writer.add_summary(loss_val, count)
        #    writer.add_summary(num_tii, count)
        #    writer.add_summary(num_itt, count)
        if count%5000 == 0:
            save_path = saver.save(sess, "models/run_3/model"+str(count)+".ckpt")
